...
create TRIGGER insereLog after insert on curso for each row
  begin
  insert into log(nome_tabela, log, chave, data, cod_usuario)
  values
  ('curso',concat('inseriu Coord: ', new.cod_usuario_coordenador), concat('codigo=', new.codigo), now(), new.cod_usuario_alteracao);
  end //

DELIMITER //
create procedure inserirTabelaLog (cod_usuario_coordenador int, codigo int,cod_usuario_alteracao)
  begin